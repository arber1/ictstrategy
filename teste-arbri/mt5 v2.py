...
class SMCTrader:
    ...
    def calculate_nearest_liquidity(self, swing_data, current_price):
        """Find nearest support/resistance levels from swing points"""
        try:
            # Get all swing highs/lows within lookback period
            swing_highs = swing_data[swing_data['HighLow'] == 1]['Level'].values
            swing_lows = swing_data[swing_data['HighLow'] == -1]['Level'].values
            ...
            # Find nearest levels
            nearest_resistance = resistance_levels.min() if resistance_levels.size > 0 else None
            nearest_support = support_levels.max() if support_levels.size > 0 else None

            # Default to percentage-based levels if none found
    ...
    def generate_signals(self, df):
        try:
            ...
            swing_data = smc.swing_highs_lows(df.tail(500), swing_length=50)

            # Get liquidity levels
            sl_level, tp_level = self.calculate_nearest_liquidity(swing_data, current_price)

            # Store in DataFrame for execution
...
def execute_trade(signal, historical_data):
    ...
    try:
        ...
        last_swing_low = swing_data[swing_data['HighLow'] == -1]['Level'].iloc[-1]
    except IndexError:
